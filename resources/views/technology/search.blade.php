...
<div class="container">
    ...
    <div class="card mb-3">
        <div class="row no-gutters">
            <div class="col-md-4">
                @if ($t->image)
                <img src="{{ asset('storage/'. $t->image) }}" class="img-fluid w-100" style="height:200px;"
                    alt="world-news" />
                @else
                <img src="https://source.unsplash.com/1200x600?{{ $t->category->name }}" class="img-fluid "
                    alt="world-news" />
                @endif
            </div>
            <div class="col-md-8">
                <div class="card-body">
                    ...
                    <small class="text-primary"><a href="/technology/searchnewstechnology?user={{ $t->user->username }}">{{
                            $t->user->name }}</a> | <a
                            href="/technology/searchnewstechnology?category={{ $t->category->slug }}">{{ $t->category->name
                            }}</a></small>
                    <p class="card-text mt-3">{{ $t->excerpt }}</p>
                    <p class="card-text"><small class="text-muted">Last updated {{ $t->created_at->diffForHumans()
                            }}</small></p>
                </div>
            </div>
        </div>
