...
<div class="container">
    ...
    <div class="card mb-3">
        <div class="row no-gutters">
            <div class="col-md-4">
                @if ($educate->image)
                <img src="{{ asset('storage/'. $educate->image) }}" class="img-fluid w-100" style="height:200px;"
                    alt="world-news" />
                @else
                <img src="https://source.unsplash.com/1200x600?{{ $educate->category->name }}" class="img-fluid "
                    alt="world-news" />
                @endif
            </div>
            <div class="col-md-8">
                <div class="card-body">
                    ...
                    <small class="text-primary"><a href="/education/searchnewseducation?user={{ $educate->user->username }}">{{
                            $educate->user->name }}</a> | <a
                            href="/education/searchnewseducation?category={{ $educate->category->slug }}">{{ $educate->category->name
                            }}</a></small>
                    <p class="card-text mt-3">{{ $educate->excerpt }}</p>
                    <p class="card-text"><small class="text-muted">Last updated {{ $educate->created_at->diffForHumans()
                            }}</small></p>
                </div>
            </div>
        </div>
