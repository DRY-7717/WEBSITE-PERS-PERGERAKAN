...
<div class="container">
    ...
    <div class="card mb-3">
        <div class="row no-gutters">
            <div class="col-md-4">
                @if ($politic->image)
                <img src="{{ asset('storage/'. $politic->image) }}" class="img-fluid w-100" style="height:200px;"
                    alt="world-news" />
                @else
                <img src="https://source.unsplash.com/1200x600?{{ $politic->category->name }}" class="img-fluid "
                    alt="world-news" />
                @endif
            </div>
            <div class="col-md-8">
                <div class="card-body">
                    ...
                    <small class="text-primary"><a href="/politic/searchnewspolitic?user={{ $politic->user->username }}">{{
                            $politic->user->name }}</a> | <a
                            href="/politic/searchnewspolitic?category={{ $politic->category->slug }}">{{ $politic->category->name
                            }}</a></small>
                    <p class="card-text mt-3">{{ $politic->excerpt }}</p>
                    <p class="card-text"><small class="text-muted">Last updated {{ $politic->created_at->diffForHumans()
                            }}</small></p>
                </div>
            </div>
        </div>
